...
class Absensi extends CI_Controller
{
    ...
    public function absen()
    {
        if (@$this->uri->segment(3)) {
            $ket = ucfirst($this->uri->segment(3));
        }
        ...
        $cek_absen = $this->absensi->absen_harian_user($this->session->id_user)->row();
       
        $id_absen = $cek_absen->id_absen;
        if($cek_absen0){
            $result = $this->absensi->update_data($id_absen, $data);
        }
        else{
    ...
    public function hari_bulan($bulan,$tahun)
    {
        ...
        $bulan_now = idate('m');
        $tahun_now = idate('Y');
        if($bulan_now != $bulan || $tahun_now != $tahun) {
